...
import type { FieldMeta } from "@/types/advanced-formatting";
...
/** 通用元数据驱动字段渲染器（解耦于具体 schema） */
export function FormattingFieldRenderer({ meta, value, disabled, onChange }: Props) {
  const key = meta.key;
      ...
      <div className="space-y-1" aria-disabled={disabled}>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            const next = typeof opt?.value === "number" ? Number(raw) : raw;
            onChange(key, next);
          }}
